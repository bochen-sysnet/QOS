...
def evolved_cost_search(self, q: Qernel, size_to_reach: int, budget: int):
    ...
    # OE_BEGIN
    timeout_sec = 0
    gv_cost, _gv_timed_out = compute_gv_cost(q, size_to_reach, timeout_sec=timeout_sec)
    wc_cost, _wc_timed_out = compute_wc_cost(q, size_to_reach, timeout_sec=timeout_sec)
    while (gv_cost <= budget or wc_cost <= budget) and size_to_reach > 2:
        ...
        gv_cost, _gv_timed_out = compute_gv_cost(
            q, size_to_reach, timeout_sec=timeout_sec
        )
