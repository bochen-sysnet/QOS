...
def evaluate(program_path):
    ...
    if evolved_run is None:
        return EvaluationResult(metrics={"combined_score": 0.0}, artifacts={"error": "Missing evolved_run"})
    ...
    if unknown:
        return EvaluationResult(
            ...
            artifacts={"error": f"Unknown benches: {', '.join(unknown)}"},
        )
    ...
    if baseline_mode not in {"qos", "qiskit"}:
        return EvaluationResult(
            metrics={"combined_score": 0.0},
            artifacts={"error": f"Unknown QOSE_BASELINE: {baseline_mode}"},
        )
    ...
    if count == 0:
        return EvaluationResult(metrics={"combined_score": 0.0}, artifacts={"error": "No benches/sizes"})
